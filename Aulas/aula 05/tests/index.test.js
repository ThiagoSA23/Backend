...
test("2 - 0 = 2", () => {
    expect(calculadora.sub(2, 0)).toBe(2);
});
...
test("se a e b < 0 ou a e b > 0 então a * b > 0", () => {
    expect(calculadora.multi).toBeDefined();
    expect(calculadora.multi(2, 2)).toBeGreaterThan(0);
    expect(calculadora.multi(-1, -2)).toBeGreaterThan(0);
});

test("se a < 0 ou b < 0 então a * b < 0", () => {
    expect(calculadora.multi(-4, 2)).toBeLessThan(0);
    expect(calculadora.multi(-1, 6)).toBeLessThan(0);
});
